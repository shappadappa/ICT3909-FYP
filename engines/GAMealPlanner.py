...
class GAMealPlanner:
    def __init__(
        ...
        :param budget_penalty_multiplier: multiplier for calculating the penalty score for exceeding the budget (higher = worse)
        :type budget_penalty_multiplier: float
        :param calorie_penalty_weight: weight applied to the absolute difference between daily calories and the calorie target (higher = stricter calorie adherence)
        :type calorie_penalty_weight: float
        :param protein_penalty_weight: weight applied to the absolute difference between daily protein and the protein target (higher = stricter protein adherence)
    ...
    def evaluate_meal_plan(
        ...
        for ingredient_name, quantity_available in pantry_stock.items():
            ...
            waste_penalty_total += self._get_waste_penalty(quantity_unused, days_until_expiry.get(ingredient_name, 999))

        budget_penalty_total = self._get_budget_penalty(purchase_cost, preferences.weekly_budget)
        ...
        for day in range(num_days):
            ...
            dietary_penalty_total += (
                abs(daily_calories - preferences.calorie_target_per_day) * self.calorie_penalty_weight
            )
            dietary_penalty_total += (
                abs(daily_protein - preferences.protein_target_per_day) * self.protein_penalty_weight
            )
