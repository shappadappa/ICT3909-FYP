...
class PantryIngredient(Ingredient):
    def __init__(
        ...
        """
        `PantryIngredient` class extends `Ingredient` with additional attributes specific to ingredients stored in the
        pantry, such as estimated expiration date.
        ...
        :param estimated_expiration_date: estimated expiration date of the ingredient (default = datetime(9999, 12, 31),
            meaning it does not expire)
    ...
    def is_expired(self, current_date: datetime | None = None) -> bool:
        ...
        :param current_date: current date to compare with the estimated expiration date (default = None, uses current
            system date)
