...
class FoodEmbedding(Embedding):
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        """
        The `FoodEmbedding` class is responsible for creating embeddings for food descriptions and allowing for
        similarity search based on those embeddings. It also provides functionality to retrieve nutritional information
        for specific food items.

        The usage of this embedding allows to bypass the API limitations of 1000 requests per hour by precomputing the
        embeddings for all food items and storing them in a FAISS index, which can be searched efficiently without
        making API calls. This allows for fast retrieval of similar food items based on their descriptions, as well as
        access to their nutritional information without hitting API rate limits
